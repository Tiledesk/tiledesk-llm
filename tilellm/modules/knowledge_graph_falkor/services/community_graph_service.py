...
class CommunityGraphService:
    ...
    async def create_community_graph(
        ...
        Args:
            ...
            index_name: Index name for filtering
            output_dir: Optional output directory for artifacts
            overwrite: Whether to overwrite existing graph data
            import_to_graph: Whether to import entities/relationships to graph
            save_to_minio: Whether to save artifacts to MinIO
    ...
    async def cleanup_before_clustering(
        self,
        namespace: str,
        ...
        This ensures a clean slate when regenerating clusters by:
        1. Deleting community report embeddings from vector store
        2. Deleting BELONGS_TO_COMMUNITY relationships from graph
        3. Deleting CommunityReport nodes from graph

        Entities and RELATED_TO relationships are preserved.
        ...
        """
        logger.info(f"Starting cleanup before clustering for namespace={namespace}, index_name={index_name}")
        ...
        if vector_store_repo and engine:
            try:
                ...
                if hasattr(vector_store_repo, 'delete_namespace'):
                    await vector_store_repo.delete_namespace(namespace_to_delete=namespace_to_delete)
                    stats["vector_store_deleted"] = 1
                    logger.info(f"Successfully deleted vector store namespace: {report_namespace}")
                else:
                    logger.warning("Vector store does not support namespace deletion")
            except Exception as e:
                logger.error(f"Failed to delete vector store namespace: {e}", exc_info=True)
        else:
            logger.warning("Vector store or engine not provided, skipping vector store cleanup")

        # 2. Clean Graph Database (BELONGS_TO_COMMUNITY relationships and CommunityReport nodes)
        if self.graph_service:
            try:
                ...
                if count_result and len(count_result) > 0:
                    rels_count = count_result[0].get("count", 0)
                ...
                if count_result and len(count_result) > 0:
                    nodes_count = count_result[0].get("count", 0)
                ...
                logger.error(f"Failed to clean up graph database: {e}", exc_info=True)
        else:
            logger.warning("GraphService not initialized, skipping graph cleanup")
            ...
            if result:
                for row_dict in result:
                    records.append({
                        ...
                        "rating_explanation": row_dict.get("rating_explanation", ""),
                        "full_report": row_dict.get("full_report", ""),
                        "import_timestamp": row_dict.get("import_timestamp", ""),
    ...
    async def context_fusion_search(
        ...
        if local_chunks and weights["graph"] > 0.3:
            try:
                chunk_ids = [c["id"] for c in local_chunks] # Expand from top 10 chunks
                logger.info(f"Graph expansion enabled - graph weight: {weights['graph']}, chunks: {len(local_chunks)}")
                ...
                logger.info(f"Total seed nodes found: {len(seed_ids)} from {len(chunk_ids)} chunks")
                ...
                if seed_ids:
                    ...
                    logger.warning(f"No seed nodes found - graph expansion skipped")
            except Exception as e:
                logger.error(f"Graph expansion failed: {e}", exc_info=True)
        else:
            if not local_chunks:
                logger.info("No local chunks available for graph expansion")
            else:
                logger.info(f"Graph expansion skipped - graph weight too low: {weights['graph']} <= 0.3")
        ...
        if all_docs:
            try:
                ...
                reranked = reranker.rerank_documents(question, all_docs, top_k=max_results) # Top 20 relevant pieces
