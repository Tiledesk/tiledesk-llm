...
try:
    from tilellm.modules.task_executor.tasks import (
        task_falkor_add_document,
        task_falkor_louvain_cluster,
...
# ==================== ROUTER SETUP ====================
router = APIRouter(
    prefix="/api/kg-falkor",
    tags=["Knowledge Graph (FalkorDB)"]
)
...
async def create_node(node: Node):
    ...
    try:
        return await kg_logic.create_node(node)
...
async def get_node(node_id: str):
    ...
    try:
        node = await kg_logic.get_node(node_id)
        if not node:
...
async def update_node(node_id: str, node_update: NodeUpdate):
    ...
    try:
        updated_node = await kg_logic.update_node(node_id, node_update)
        if not updated_node:
            raise HTTPException(status_code=404, detail=f"Node with ID '{node_id}' not found")
        return updated_node
    except HTTPException:
...
async def patch_node(node_id: str, node_update: NodeUpdate):
    """Partially update a node's properties (alias for PUT)."""
    return await update_node(node_id, node_update)
...
async def delete_node(
    ...
    try:
        deleted = await kg_logic.delete_node(node_id, detach)
        if not deleted:
            raise HTTPException(status_code=404, detail=f"Node with ID '{node_id}' not found")
        return None
    except HTTPException:
...
async def create_relationship(relationship: Relationship):
    ...
    try:
        return await kg_logic.create_relationship(relationship)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_relationship(relationship_id: str):
    ...
    try:
        relationship = await kg_logic.get_relationship(relationship_id)
        if not relationship:
...
async def get_node_relationships(
    ...
    try:
        return await kg_logic.get_node_relationships(node_id, direction)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def update_relationship(relationship_id: str, relationship_update: RelationshipUpdate):
    """Update a relationship's type and/or properties."""
    try:
        updated_rel = await kg_logic.update_relationship(relationship_id, relationship_update)
        if not updated_rel:
            raise HTTPException(status_code=404, detail=f"Relationship with ID '{relationship_id}' not found")
        return updated_rel
    except HTTPException:
...
async def patch_relationship(relationship_id: str, relationship_update: RelationshipUpdate):
    """Partially update a relationship's properties (alias for PUT)."""
    return await update_relationship(relationship_id, relationship_update)

@router.delete("/relationships/{relationship_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_relationship(relationship_id: str):
    """Delete a relationship from the graph."""
    try:
        deleted = await kg_logic.delete_relationship(relationship_id)
        if not deleted:
            raise HTTPException(status_code=404, detail=f"Relationship with ID '{relationship_id}' not found")
        return None
    except HTTPException:
...
async def graph_create(request: GraphCreateRequest):
    ...
    try:
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            ...
            task = await task_falkor_graph_create.kiq(payload)
...
async def add_document(request: AddDocumentRequest):
    ...
    **Example Request:**
    ...
    try:
        ...
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            payload = serialize_with_secrets(request.model_dump(mode='python'))
            task = await task_falkor_add_document.kiq(payload)
            return AsyncTaskResponse(task_id=task.task_id)
...
async def graph_cluster_louvain(request: GraphClusterRequest):
    ...
    try:
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            payload = serialize_with_secrets(request.model_dump(mode='python'))
            task = await task_falkor_louvain_cluster.kiq(payload)
            return AsyncTaskResponse(task_id=task.task_id)
...
async def graph_cluster_leiden(request: GraphClusterRequest):
    ...
    try:
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            payload = serialize_with_secrets(request.model_dump(mode='python'))
            task = await task_falkor_leiden_cluster.kiq(payload)
            return AsyncTaskResponse(task_id=task.task_id)
...
async def graph_cluster_hierarchical(request: GraphClusterRequest):
    ...
    try:
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            payload = serialize_with_secrets(request.model_dump(mode='python'))
            task = await task_falkor_hierarchical_cluster.kiq(payload)
            return AsyncTaskResponse(task_id=task.task_id)
...
async def get_graph_network(
    ...
    **Example Usage:**
    ...
    """
    try:
        result = await kg_logic.get_graph_network(
            namespace=namespace,
...
@router.post("/advancedqa", response_model=GraphQAAdvancedResponse, tags=["Search Methods"])
async def advanced_qa_search(request: GraphQAAdvancedRequest):
    """
    **METHOD 4: Advanced QA Search (Domain-Specific)**

    Specialized search for debt collection (recupero crediti) domain.

    **Pipeline:**
    1. **Intent Classifier** → identifies query type (timeline, esposizione, garanzie, pagamenti, etc.)
    2. **Parameter Extractor** → extracts entities (debtor_name, loan_id, date_range, etc.)
    3. **Vector Store Search** → finds relevant seed nodes
    4. **Community Reports** → retrieves max 3 relevant community reports
    5. **Template Cypher Engine** → generates safe parameterized graph queries
    6. **Graph Execution** → executes queries on FalkorDB
    7. **LLM Response Enricher** → transforms raw results into readable response

    **Supported Intent Types:**
    - `timeline`: Sequence of events for a debtor/loan
    - `exposure`: Total debt amount and breakdown
    - `guarantees`: Collaterals and securities information
    - `payments`: Payment history and transactions
    - `contacts`: Communications with debtor
    - `legal_actions`: Legal proceedings status
    - `debtor_info`: Debtor profile and related information
    - `relationship`: Connections between entities

    **Input:** Same as `/hybrid` endpoint (GraphQAAdvancedRequest)
    **Output:** Same as `/hybrid` endpoint (GraphQAAdvancedResponse)

    **Example Query:**
    ```
    "Qual è la cronologia degli eventi per il debitore Mario Rossi?"
    "Mostrami l'esposizione totale per la pratica PR-2024-001"
    "Quali garanzie sono attive per Giovanni Bianchi?"
    ```

    **Key Differences from /hybrid:**
    - Always uses max 3 community reports (optimized for focused context)
    - Generates domain-specific Cypher queries based on detected intent
    - Structured parameter extraction for precise graph navigation
    - Intent-aware response formatting (timeline, financial summary, etc.)
    """
    try:
        result = await kg_logic.advanced_qa_search(request)
        ...
        return GraphQAAdvancedResponse(
            ...
            relationships=result.get("relationships", []),
            query_used=request.question if isinstance(request.question, str) else "",
            retrieval_strategy=result.get("retrieval_strategy", "advanced_qa"),
            scores=result.get("scores", {}),
            expanded_nodes=result.get("expanded_nodes", []),
            expanded_relationships=result.get("expanded_relationships", []),
            chat_history_dict=result.get("chat_history_dict")
        ...
        logger.error(f"Advanced QA search failed: {e}")
        raise HTTPException(status_code=500, detail=f"Advanced QA search failed: {str(e)}")
...
async def community_analysis(request: GraphClusterRequest):
    ...
    **Analysis: Document Community Detection**
    ...
    try:
        if ENABLE_TASKIQ and TASKIQ_AVAILABLE:
            payload = serialize_with_secrets(request.model_dump(mode='python'))
            task = await task_falkor_community_analysis.kiq(payload)
            return AsyncTaskResponse(task_id=task.task_id)
...
@router.post("/agenticqa", response_model=GraphQAAdvancedResponse, tags=["Search Methods"])
async def agentic_qa_search(request: GraphQAAdvancedRequest):
    """
    **METHOD 5: Agentic QA Search (The Graph Specialist)**

    Agentic approach where an LLM agent autonomously queries the knowledge graph using Cypher.
    
    **Features:**
    - **Self-Correction**: Retries failed queries.
    - **Direct Graph Access**: Agent writes its own Cypher queries.
    - **Summarization**: Handles large result sets via Map-Reduce.
    
    **Use Cases:**
    - Complex questions not covered by standard templates.
    - "Who are the guarantors?" (Precise MATCH)
    - "Count all protests" (Aggregation)
    - "Reconstruct timeline" (Custom ORDER BY)
    """
    try:
        result = await kg_logic.agentic_qa_search(request)
        ...
        return GraphQAAdvancedResponse(
            ...
            relationships=result.get("relationships", []),
            query_used=request.question if isinstance(request.question, str) else "",
            retrieval_strategy=result.get("retrieval_strategy", "agentic_qa"),
            scores=result.get("scores", {}),
            expanded_nodes=result.get("expanded_nodes", []),
            expanded_relationships=result.get("expanded_relationships", []),
            chat_history_dict=result.get("chat_history_dict")
