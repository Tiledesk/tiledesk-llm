...
class GraphExpander:
    ...
    async def expand_from_seeds(
        ...
        for hop in range(max_hops):
            ...
            logger.debug(f"Expansion query:\n{query}")
            logger.debug(f"Query params: current_nodes={params.get('current_nodes')[:3] if params.get('current_nodes') else []}, "
                        f"namespace={params.get('namespace')}, index_name={params.get('index_name')}")
    ...
    def _build_expansion_query(
        ...
        # Conditions - filter both source and target nodes by namespace/index
        conditions = ["id(n) IN $current_nodes", "NOT id(m) IN $already_expanded"]

        if namespace:
            # Filter BOTH source and target nodes by namespace for multi-graph isolation
            conditions.append("n.namespace = $namespace")
            conditions.append("m.namespace = $namespace")

        if index_name:
            # Filter BOTH source and target nodes by index_name
            conditions.append("n.index_name = $index_name")
        ...
        # Return - use id() for node identifiers
        query_parts.append("""
            RETURN
                id(m) as target_id,
                labels(m)[0] as label,
        ...
        # Parameters - Convert IDs to integers for FalkorDB
        # FalkorDB id() returns integers, so we need to match the type
        current_nodes_int = [int(nid) if isinstance(nid, str) else nid for nid in current_nodes]
        already_expanded_int = [int(nid) if isinstance(nid, str) else nid for nid in already_expanded]

        params = {
            "current_nodes": current_nodes_int,
            "already_expanded": already_expanded_int,
        }
