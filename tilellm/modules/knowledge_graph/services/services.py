...
class GraphService:
    ...
    def create_node(self, node: Node, namespace: Optional[str] = None, index_name: Optional[str] = None, engine_name: Optional[str] = None, engine_type: Optional[str] = None, metadata_id: Optional[str] = None) -> Node:
        ...
        return self._get_repository().create_node(node, namespace=namespace, index_name=index_name, engine_name=engine_name, engine_type=engine_type, metadata_id=metadata_id)
    ...
    def find_entity_by_name_and_type(self, entity_name: str, entity_type: str, namespace: Optional[str] = None, index_name: Optional[str] = None) -> Optional[Node]:
        """
        ...
        """
        matching_nodes = self.search_nodes(
            label=entity_type,
            property_key="name",
    ...
    def create_relationship(self, relationship: Relationship, namespace: Optional[str] = None, index_name: Optional[str] = None, engine_name: Optional[str] = None, engine_type: Optional[str] = None, metadata_id: Optional[str] = None) -> Relationship:
        ...
        return self._get_repository().create_relationship(relationship, namespace=namespace, index_name=index_name, engine_name=engine_name, engine_type=engine_type, metadata_id=metadata_id)
    ...
    def get_graph_network(
        ...
        for node in nodes:
            if not node.id:
                continue
            try:
                node_rels = repo.find_relationships_by_node(
                    node_id=node.id,
                    direction="both"
                )
...
class GraphRAGService:
    ...
    async def import_from_vector_store(self, namespace: str, vector_store_repo=None, engine: Optional[Engine] = None, limit: int = 100, llm=None, index_name: Optional[str] = None, overwrite: bool = True, engine_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        if overwrite:
            ...
            graph_repo.delete_nodes_by_metadata(namespace=namespace, index_name=index_name, engine_name=engine_name, engine_type=engine_type)
                ...
                try:
                    ...
                    for entity in entities:
                        try:
                            ...
                            created_node = self.graph_service.create_node(node,
                                                                          namespace=namespace,
                                                                          index_name=index_name,
                                                                          engine_name=engine_name,
                                                                          engine_type=engine_type,
                    ...
                    for rel in relationships:
                        try:
                            ...
                            self.graph_service.create_relationship(relationship, namespace=namespace, index_name=index_name, engine_name=engine_name, engine_type=engine_type, metadata_id=rel_metadata_id)
    ...
    async def _create_simple_document_nodes(self, chunks, namespace: str, index_name: Optional[str] = None, engine_name: Optional[str] = None, engine_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        for chunk in chunks:
            ...
            try:
                ...
                self.graph_service.create_node(node, namespace=namespace, index_name=index_name, engine_name=engine_name, engine_type=engine_type, metadata_id=metadata_id)
                nodes_created += 1
            except Exception as e:
        ...
        for entity in entities:
            try:
                ...
                if deduplicate_entities:
                    existing_node = self.graph_service.find_entity_by_name_and_type(
                ...
                if existing_node:
                    ...
                    created_node = self.graph_service.create_node(
                        node,
                        namespace=namespace,
                        index_name=index_name,
                        engine_name=engine_name,
